...
FROM AL WHERE AL_SROWCODE = 'W01' AND AL_SRCODE = '1234567890';
  ...
  */
  ...
  AS
  SELECT TO_DATE(TO_CHAR(OD.OD_OSDATE, 'YYMMDD'), 'YYMMDD') AS ODDATE, 
  ...
  FROM OD RIGHT OUTER JOIN SP ON OD.OD_SPPRCODE = SP.SP_PRCODE AND
                           OD.OD_OSOWCODE = SP.SP_OWCODE
          INNER JOIN PR ON OD.OD_SPPRCODE = PR.PR_CODE
  GROUP BY OD.OD_OSOWCODE, OD.OD_OSSRCODE, 
           OD.OD_OSMECMCODE, TO_DATE(TO_CHAR(OD.OD_OSDATE, 'YYMMDD'), 'YYMMDD'),
           OD.OD_SPPRCODE, PR.PR_NAME, SP.SP_COST, SP.SP_PRICE;
...
-- 
SELECT OD.OD_SPPRCODE, SP.SP_PRCODE
FROM OD RIGHT OUTER JOIN SP ON OD.OD_SPPRCODE = SP.SP_PRCODE AND 
                               OD.OD_OSOWCODE = SP.SP_OWCODE
WHERE OD.OD_OSOWCODE = 'W01' AND OD.OD_OSSRCODE = '1234567890'
GROUP BY OD.OD_SPPRCODE, SP.SP_PRCODE;