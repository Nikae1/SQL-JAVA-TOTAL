					...
					if(menuSelection.equals("0")) {
						...
						if(menuSelection.equals("1")) {
							...
							String[] userSelection = { 
									"    ++++++++++++++++++++++++++++++++++++ Select Date : ", 
									"    ─────────────────────────────────── Confirm(Y/N) : "};
							String[][] item = {{"TIME Schedule(HHmmss)      : ", null}, {"ITEM CONTENT(THINGS TO DO) : ", null}, 
						...
						/*
						 *    개발자      : 김지웅
						 * 메서드기능설명    :  3번 MODIFY기능을 수행하기 위한 구문 
						 * 파라미터 선정이유 :
						 * Return Type 선정이유 : 
						 * */
							...
							String [] selectValue = {null, null, null, null, null, null};
							String[] userSelection = { 
									"    ++++++++++++++++++++++++++++++++++++ Select Date : ", 
									"    ─────────────────────────────────── Confirm(Y/N) : ",
									"    ____________________________________ Q : 뒤로가기"};
							String [] modify = {"StartDate", "EndDate", "Contents",
						...
						/* 날짜의범위 값이 아닌 경우 재입력 요구 */
//							if(this.isInteger(menuSelection)) {
//								if(this.isIntegerRange(Integer.parseInt(menuSelection), 0, 31)) {
//									break;
//								}else {
//									message = "달력 범위내로 선택해주세요";
//								}
//							}else {
//								message = "숫자로 입력해주세요";
//							}					
...
//						System.out.println(selectValue[0]);
							/* SERVICECODE=31&STARTDATE=********&ACCESSCODE=WOONG */
							String clientData = this.makeClientData("31", modify[0], selectValue)+ "&accessCode=" + accessInfo[0];
							this.display(mainTitle);
							this.display(task.taskController(clientData).toString());	
							run = false;
						}
//							ctl.controller(this.makeClientData("-1", itemName, accessInfo));
			...
			if(!menuSelection.equals("I"))
				continue;
				if(!menuSelection.toUpperCase().equals("S"))
					continue;
					if(!menuSelection.toUpperCase().equals("E"))
						continue;
						if(!menuSelection.toUpperCase().equals("P"))
							continue;
							if(!menuSelection.toUpperCase().equals("N"))
								continue;
								if(!menuSelection.toUpperCase().equals("F"))
			break;
	...
	/* MODIFY를 위한 CLIENTDATA */
	private String makeClientData(String serviceCode, String modify, String[] selectValue) {
		StringBuffer clientData = new StringBuffer();
		clientData.append("serviceCode=" + serviceCode);
		for(int idx = 0; idx<selectValue.length; idx++) {
			if(selectValue[idx] !=null) {
				clientData.append("&");
				clientData.append(modify+ "="+ selectValue);
			}
		}
		return clientData.toString();
	}
	...
	private String mainTitle(String date) {
		...
		title.append("     ◀▷◀▷◀▷◀▷◀▷◀▷◀▷◀▷◀▷        Designed by Wooooooong\n");
		title.append("  __________________________________________________________\n");
